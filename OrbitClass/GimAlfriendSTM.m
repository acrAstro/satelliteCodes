...
%% The Gim-Alfriend State Transition Matrix
%
% This class contains the code to propagate the Gim-Alfriend STM, a
% geometric method for modeling the relative effect of J2 to first order on
% the relative motion of two satellites in an elliptical orbit.
    properties
        Phi                     % Phi is the GASTM
        time                    % Time vector
        J2                      % J2: 1082.63e-6 or zero
        ChiefOsc                % Osculating chief orbital elements
        Req                     % Radius of Earth, 6378.137 km (in meters)
        mu                      % Gravitational parameter, 3.986e14 m^3/s^2
        tol                     % Tolerance for Kepler's equation solver
        ChiefElemsNSMean        % Chief mean, nonsingular elements
        chiefOrbitDescription   % Chief orbit description (string)
        deputyOrbitDescription  % Deputy orbit description (string)
        initialConditions       % Initial deputy relative state
        kepElemsInit            % Initial chief Kepler elements
        DepElemsInit            % Initial deputy Kepler elements
        DepElemsInitNS          % Initial nonsingular deputy elements
        DepOsc                  % Initial deputy osculating elements
        X                       % State history
        t0                      % Initial time
        numPeriod               % Number of orbit periods for simulation
        safetyAltitude          % Altitude below wich we say the orbit dies
        period                  % Length of orbital period
    end
        ...
        function GA = GimAlfriendSTM(initStruct)
            ...
            switch method
                ...
                case 'Nonsingular'
                    ...
                    [~,GA.ChiefOsc] = MeanToOsculatingElements(GA.J2,GA.ChiefElemsNSMean,GA.Req,GA.mu);
                case 'RV'
                    GA.kepElemsInit = rv2oe(initStruct.Elements,GA.mu);
            end
